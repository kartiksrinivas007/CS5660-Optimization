...
def return_loader(args):
    if (args.dataset == "cifar10"):
        ...
        cifar10_transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize(mean=cifar10_mean, std=cifar10_std),
             transforms.CenterCrop(32)])  # need to normalize
    ...
    if (args.dataset == "magic04s"):
        ...
        magic04s_train_X, magic04s_test_X, magic04s_val_X, magic04s_train_y, magic04s_test_y, magic04s_val_y = split_data(
            magic04s_X, magic04s_y, args)
        magic04s_trainloader = tud.DataLoader(
            dataset.TensorDataset(torch.tensor(magic04s_train_X), torch.tensor(magic04s_train_y)),
            batch_size=args.batch_size, shuffle=True)
        magic04s_testloader = tud.DataLoader(
            dataset.TensorDataset(torch.tensor(magic04s_test_X), torch.tensor(magic04s_test_y)),
            batch_size=args.batch_size, shuffle=False)
        magic04s_valoader = tud.DataLoader(
            dataset.TensorDataset(torch.tensor(magic04s_val_X), torch.tensor(magic04s_val_y)),
            batch_size=args.batch_size, shuffle=False)
        return magic04s_trainloader, magic04s_testloader, magic04s_valoader

    if (args.dataset == "magic04d"):
        # note that this dataset by default is normalized even the test set!

        magic04d_X, magic04d_y = magic04d()
        magic04d_train_X, magic04d_test_X, magic04d_val_X, magic04d_train_y, magic04d_test_y, magic04d_val_y = split_data(
            magic04d_X, magic04d_y, args)
        magic04d_trainloader = tud.DataLoader(
            dataset.TensorDataset(torch.tensor(magic04d_train_X), torch.tensor(magic04d_train_y)),
            batch_size=args.batch_size, shuffle=True)
        magic04d_testloader = tud.DataLoader(
            dataset.TensorDataset(torch.tensor(magic04d_test_X), torch.tensor(magic04d_test_y)),
            batch_size=args.batch_size, shuffle=False)
        magic04d_valoader = tud.DataLoader(
            dataset.TensorDataset(torch.tensor(magic04d_val_X), torch.tensor(magic04d_val_y)),
            batch_size=args.batch_size, shuffle=False)
